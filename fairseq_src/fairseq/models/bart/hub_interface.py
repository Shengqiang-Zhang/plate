...
# hypo_attn = Path("teacher_attn.txt")
# print("write attn file in", str(hypo_attn.resolve()))
# f_attn = hypo_attn.open("w", encoding="utf-8")
...
class BARTHubInterface(nn.Module):
    ...
    def generate(self, tokens: List[torch.LongTensor], beam: int = 5, verbose: bool = False, **kwargs) -> torch.LongTensor:
        ...
        # -----modified by shengqiang------
        # attn_cpu = hypos[0]["attention"].cpu()
        # np.savetxt(f_attn, attn_cpu.detach().numpy())
        # f_attn.write("\n")
